...
S10.0, S10.1 e S10.2 estao concluidos; o macro-stage permanece em andamento. O Lumen aceita ingest autenticado M2M de watcher-event metadata-only e o persiste idempotentemente, criando evidence minima apenas quando empresa e periodo forem resolvidos. `classifier_hint` de filename e somente sinal auxiliar preservado no payload: `detected_tax` e `detected_obligation` permanecem nulos ate parser de conteudo futuro. A regra de S11 sera `PARSER DE CONTEUDO > NOME DO ARQUIVO > CONTEXTO AUXILIAR`. Ele ainda nao observa `G:\EMPRESAS` continuamente, nao possui client HTTP do agent, nao processa conteudo PDF, nao faz OCR, nao executa parsers fiscais e nao concilia obrigacao. S11 e S12 nao foram iniciados.
...
O watcher fiscal possui heartbeat M2M sanitizado, estado humano read-only e reprocessamento administrativo de eventos ainda sem evidence. Nenhuma tela inicia watcher, envia arquivo ou classifica tributo. A fase 1 do piloto real controlado foi concluida na root oficial: o primeiro baseline encontrou `1385` candidatos, sem criar eventos ou evidencias; um segundo `--once` encerrou o heartbeat remoto em `STOPPED`.
...
O fechamento automatizado cobre health humano, reprocessamento local idempotente e downgrade/upgrade da migration de health. S10.4 permanece em piloto manual por fases e S10 permanece em andamento; nenhum piloto e iniciado automaticamente pelo repositorio.
