...
O agent continua por polling incremental, com baseline sem envio historico, state/health atomicos, estabilidade por tamanho/mtime e retry limitado. Na primeira execucao com cobertura S11.0, candidatos antes invisiveis sao apenas baselinados sem apagar o state S10. O scanner enumera empresas imediatamente abaixo da enterprise root e desce somente na child `Escrita Fiscal`; symlinks, junctions e reparse points nao sao seguidos. O agent envia metadata M2M e nao promove filename, pasta, `Matriz` ou `Filial` a classificacao ou empresa final.
...
XML e ZIP agora podem ser descobertos como candidatos tecnicos, mas NFS-e permanece fonte fiscal separada para S11.3. S11.0 nao interpreta XML, nao extrai ZIP e nao determina atividade, receita, Fator R ou estabelecimento.
## S10.4 Watcher fiscal

O watcher fiscal possui heartbeat M2M sanitizado, estado humano read-only e reprocessamento administrativo de eventos ainda sem evidence. Nenhuma tela inicia watcher, envia arquivo ou classifica tributo. A fase 1 do piloto real controlado encontrou `1385` candidatos no baseline, sem criar eventos ou evidencias; um segundo `--once` encerrou o heartbeat remoto em `STOPPED`.

A fase 2 validou um unico PDF real por operacao manual: o dry-run nao alterou o banco; o envio com `--confirm-send` retornou HTTP `200` e criou um evento `FILE_STABLE` pendente e uma evidence `WATCHER_FILE` pendente vinculada. O replay confirmado nao duplicou os registros. O piloto nao executou parser, OCR, classificacao fiscal ou alteracao de obrigacoes; campos canonicos `detected_tax` e `detected_obligation` continuaram nulos. Nomes, caminhos, hash e conteudo do arquivo real nao sao registrados no Git.
...
Em `2026-09-11`, o S11.0 foi validado integralmente com `.venv`: a suite focada do Watcher passou com `114 passed` e a regressao completa do backend com `729 passed`; ambas exibiram somente o warning conhecido de deprecacao do `TestClient`. Ruff passou sem ocorrencias. O banco permaneceu em `20260904_0017 (head)` e nenhum arquivo de migration foi criado ou alterado.

No frontend, `npm run typecheck` e `npm run build` passaram, com `62` modulos transformados no build. Como nao existe impacto visual do contrato v2, nenhum caso E2E novo foi adicionado; a regressao Playwright existente passou com `14 passed` em `1.5m`.
